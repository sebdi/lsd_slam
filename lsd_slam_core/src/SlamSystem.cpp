...
void SlamSystem::createNewCurrentKeyframe(std::shared_ptr<Frame> newKeyframeCandidate)
{
    ...
    std::cout << "setDepth with GT" << std::endl;
    //newKeyframeCandidate.get()->setDepth(GTdepth);
...
void SlamSystem::changeKeyframe(bool noCreate, bool force, float maxScore, DepthMapPixelHypothesis *depth)
{
...
void SlamSystem::trackFrame(uchar* image, unsigned int frameID, bool blockUntilMapped, double timestamp, DepthMapPixelHypothesis * depth)
{
    if (depth!=0)
    {
        //currentKeyFrame->setDepth(depth);
        //map->currentDepthMap = depth;
        GTdepth=depth;
