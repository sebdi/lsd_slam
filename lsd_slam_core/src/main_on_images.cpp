...
float* projectLidarInImage(int width, int height, std::vector<veloPoint> &velpoints)
{
    ...
    for (size_t j=0;j<velpoints.size();j++)
    {
        ...
        if (X_temp(2)>0)
        {
            ...
            {
                depth[u+v*width] = (double)X_temp(2);
                for (size_t j = 1;j<2;j++)
                {
                    depth[(u-j)+v*width] = (double)X_temp(2);
                    depth[(u+j)+v*width] = (double)X_temp(2);
                    depth[u+(v+j)*width] = (double)X_temp(2);
                    depth[u+(v-j)*width] = (double)X_temp(2);
                    depth[(u-j)+(v-j)*width] = (double)X_temp(2);
                    depth[(u+j)+(v+j)*width] = (double)X_temp(2);
...
int main( int argc, char** argv )
{
    ...
    for(unsigned int i=0;i<files.size();i++)
    {
        ...
        if(runningIDX == 0)
        {
            if (velpoints.size() >0 && velpoints[runningIDX].size()>0)
            {
                float * gtdepth = projectLidarInImage(w,h, velpoints[runningIDX]);
                system->gtDepthInit(image.data,gtdepth,fakeTimeStamp,runningIDX);
            }
        }
        else
